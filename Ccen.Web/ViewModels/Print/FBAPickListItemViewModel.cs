...
{
    public class FBAPickListItemViewModel
    {
        ...
        public FBAPickListItemViewModel()
        {
            SoldSizes = new List<PickListSizeInfo>();
            AllSizes = new List<PickListSizeInfo>();
        }

        private static IEnumerable<ListingOrderDTO> GetOrdersWithItemsForPickList(IUnitOfWork db, long fbaShipmentId)
        {
            var shipmentListings = db.FBAPickListEntries.GetAllAsDto().Where(i => i.FBAPickListId == fbaShipmentId).ToList();
            var listingIds = shipmentListings.Select(l => l.ListingId).ToList();
            var listingInfoes = db.Listings.GetViewListingsAsDto(false).Where(l => listingIds.Contains(l.Id)).ToList();

            var results = new List<ListingOrderDTO>();
            foreach (var shipmentListing in shipmentListings)
            {
                var listingInfo = listingInfoes.FirstOrDefault(l => l.Id == shipmentListing.ListingId);

                var listing = new ListingOrderDTO()
                {
                    Id = shipmentListing.Id,
                    QuantityOrdered = shipmentListing.Quantity,

                    ASIN = listingInfo.ASIN,
                    ParentASIN = listingInfo.ParentASIN,
                    SKU = listingInfo.SKU,
                    ListingId = listingInfo.ListingId,
                    Picture = listingInfo.ParentASIN,
                    Size = listingInfo.Size,
                    Color = listingInfo.Color,

                    StyleID = listingInfo.StyleString,
                    StyleImage = listingInfo.StyleImage,
                    StyleSize = listingInfo.StyleSize,
                    StyleId = listingInfo.StyleId,
                    StyleItemId = listingInfo.StyleItemId,
                };
                results.Add(listing);
            }
            return results;
        ...
        public static IList<FBAPickListItemViewModel> GetPickListItems(IUnitOfWork db, 
            ILogService log, 
            long fbaShipmentId)
        {
            ...
            var summaryItems = new List<FBAPickListItemViewModel>();
        ...
        private static void ItemPostProcessing(FBAPickListItemViewModel item,
