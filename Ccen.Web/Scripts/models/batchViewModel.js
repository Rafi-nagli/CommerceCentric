...
var BatchModel = function(gridId, model, settings) {
    var self = this;
        ...
        $.ajax({
            ...
            success: function (dataResult) {
                ...
                if (displayRow != null) {
                    ...
                    self.drawOrderRowShippingOptions(displayRow);
                    self.grid.updateRow(displayRow);
                ...
                if (!dataResult.IsSuccess) {
                    Message.popup(dataResult.Messages.length > 0 ? dataResult.Messages[0].Message : "Error update shipping options",
                        Message.POPUP_INFO);
                }
    ...
    self.filterItem = function (filters, row, index, passedCount) {
        ...
        if (!dataUtils.isEmpty(filters.shippingStatus)) {
            ...
            if (filters.shippingStatus == self.settings.unshippedOrderStatus) {
                pass = pass && (row.OrderStatus == self.settings.unshippedOrderStatus
                    || row.OrderStatus == self.settings.partiallyShippedOrderStatus);
            }
