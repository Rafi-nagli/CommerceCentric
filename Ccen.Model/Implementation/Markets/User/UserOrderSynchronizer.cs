...
{
    public class UserOrdersSynchronizer : BaseOrdersSynchronizer
    {
        public UserOrdersSynchronizer(ILogService log,
            CompanyDTO company,
        ...
        protected SyncResult UpdateExistingUnshippedOrders(CancellationToken? token,
            ...
            ITime time)
        {
            List<DTOOrder> updatedOrders = new List<DTOOrder>();

            try
            {
                IList<DTOOrder> existOrders;
                using (var db = new UnitOfWork())
                {
                    existOrders = db.Orders.GetOrdersByStatus(api.Market,
                        api.MarketplaceId,
                        new[]
                        {
                ...
                if (excludeOrderIdList != null)
                    existOrders = existOrders.Where(o => !excludeOrderIdList.Contains(o.OrderId)).ToList();

                log.Info("Orders to update count:" + existOrders.Count
                    + " (existing in DB=" + existOrders.Count
                    + ", exclude list=" + (excludeOrderIdList != null ? excludeOrderIdList.Count.ToString() : "[null]") + ")");

                var index = 0;
                var step = 50;
                while (index < existOrders.Count)
                {
                    var orderIdToProcess = existOrders.Skip(index).Take(step).Select(o => o.Id).ToList();

                    using (var db = new UnitOfWork())
                    {
                        var ordersToProcess = db.Orders.GetAll().Where(o => orderIdToProcess.Contains(o.Id)).ToList();
                        ...
                        updatedOrders.AddRange(ordersToProcess.Select(o => new DTOOrder()
                        {
                            OrderId = o.AmazonIdentifier,
                ...
                {
                    IsSuccess = true,
                    ProcessedOrders = updatedOrders.Select(o => new SyncResultOrderInfo()
                    {
                        OrderId = o.OrderId,
                        OrderStatus = o.OrderStatus
                    }).ToList()
        ...
        protected override bool FillOrderItemsBy(ILogService log, IUnitOfWork db, IMarketApi api, IList<ListingOrderDTO> orderItems)
        {
