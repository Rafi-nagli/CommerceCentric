...
{
    public class EBayOrdersSynchronizer : BaseOrdersSynchronizer
    {
        public EBayOrdersSynchronizer(ILogService log,
            IMarketApi api,
            CompanyDTO company,
            ...
            ITime time)
        {
            ...
            try
            {
                ...
                {
                    var existOrders = db.Orders.GetOrdersByStatus(_api.Market, 
                        _api.MarketplaceId,
                        new [] {
        ...
        protected override bool FillOrderItemsBy(ILogService log, 
            ...
            ) //NOTE: only using for listings lookup (on Amazon)
        {
            //Check order listings
            var allListingsUpdated = db.Listings.FillOrderItemsByListingId(orderItems, api.Market, api.MarketplaceId);
