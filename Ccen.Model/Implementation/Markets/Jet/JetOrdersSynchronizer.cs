...
namespace Amazon.Model.Implementation.Markets.Jet
{
    public class JetOrdersSynchronizer : BaseOrdersSynchronizer
    {
        public JetOrdersSynchronizer(ILogService log,
            IMarketApi api,
            CompanyDTO company,
            ...
            ITime time)
        {
            ...
            try
            {
                ...
                {
                    var existOrders = db.Orders.GetOrdersByStatus(MarketType.Jet, 
                        "",
                        new [] {
            ...
            ITime time)
        {
            try
            {
                ...
                var statusList = new List<string> { 
                    //OrderStatusEnumEx.Canceled, NOTE: Jet hasn't that order status
                ...
                if (allOrders.Any())
                {
                    _log.Info("Orders count=" + allOrders.Count);
