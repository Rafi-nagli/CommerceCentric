...
{
    public class GrouponOrdersSynchronizer : BaseOrdersSynchronizer
    {
        public GrouponOrdersSynchronizer(ILogService log,
            CompanyDTO company,
        ...
        {
            var allListingsUpdated = db.Listings.FillOrderItemsBySKUOrBarcode(orderItems, api.Market, api.MarketplaceId);
            ...
            if (allListingsUpdated)
            {
                foreach (var orderItem in orderItems)
                {
                    if (orderItem.Id > 0)
                    {
                        var dbListing = db.Listings.GetAll().FirstOrDefault(l => l.Id == orderItem.Id);
                        var newPrice = dbListing.CurrentPrice * orderItem.QuantityOrdered;
                        var shippingPrice = dbListing.ShippingPriceFromMarket;
                        log.Info("Price changed, SKU=" + orderItem.SKU + ", price=" + orderItem.ItemPaid + "=>" + newPrice);
                        //orderItem.ItemGrandPrice = newPrice;
                        orderItem.ItemTotal = newPrice;
                        orderItem.ItemPrice = newPrice;
                        orderItem.ItemPaid = newPrice;
                        if (shippingPrice.HasValue)
                        {
                            orderItem.ShippingPrice = shippingPrice.Value;
                            orderItem.ShippingPaid = shippingPrice.Value;
                        }
                    }
                }
                db.Commit();
